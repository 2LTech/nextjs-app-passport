...
import { guard, isSameOrigin } from '@/lib/api/security'
...
describe('@/lib/api/security', () => {
  ...
  describe('guard', () => {
    test('allows a same-origin POST', () => {
      expect(
        guard(makeReq('POST', { 'sec-fetch-site': 'same-origin' }))
      ).toBeNull()
    })

    test('rejects non-POST with 405 and Allow header', async () => {
      const res = guard(makeReq('GET'))
    ...
    test('rejects a cross-site POST with 403', async () => {
      const res = guard(makeReq('POST', { 'sec-fetch-site': 'cross-site' }))
      expect(res).not.toBeNull()
